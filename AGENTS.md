...
给 nono（5 岁）的**成长陪伴工作台**微信小程序，**家庭自用**，不走发布流程。
一个入口下有学习、自律、健康、奖励、宠物五件事，外加 PIN 保护的家长端。
详见 `docs/vision.md`。

功能来源是已上线的 PWA <https://xiaoluzhu.net/>（「Nono 工作台」）。
小程序版是**功能迁移**：偏离线上行为必须在对应 `doc.md` 里写明理由。
宠物只是其中一个模块 —— 本仓库 2026-08-11 之前定位为纯宠物养成，勿沿用旧描述。
...
不得自创同义词（如把「勋章」写成 `badge` / `token` / `honor`，或把「饱腹度」写成 `hunger` / `satiety`）。

`docs/glossary.md` 的标识符是**本仓库的规范命名**，与线上工作台的存档字段名不一定相同；
两者的映射写在 `docs/features/storage/doc.md`，不要把线上字段名直接当标识符用。
...
- **`miniprogram/data/` 是常量区**：只 `export` 数组或对象字面量，**不含任何函数、判断、计算**。
  内容资产（2000 汉字、169 古诗、约 200 贴纸、成就表）放这里，不放 `utils/` 也不塞进页面。
  这类文件体积大且无分支，不要求单测，也不参与 `validate:docs` 的 utils 认领检查；
  但每个 `data/` 模块必须在某份 `doc.md` 里说明它的字段结构与来源。
- **页面是适配层**：调用 `wx.*`、读写 storage、取当前时间，然后把值交给 `utils/` 的纯函数。
  页面里不写业务判断（阈值、公式、状态迁移）。
- **依赖方向单向**：`pages → utils → data`。`utils/` 不得引用 `pages/`，`data/` 不得引用任何模块。

## 4. 行为规格与 Spec ID（本仓库的核心机制）

每条业务规则在 `doc.md` 的规格表里有一个唯一 ID，格式 `<AREA>-<NN>`（大写字母区 + 两位数字），
例如 `FULLNESS-02`。测试用例标题以 `[ID]` 开头引用它：
...
| Spec ID     | 输入          | 期望输出  |
| ----------- | ------------- | --------- |
| FULLNESS-02 | 距上次喂食 6h | 饱腹度 50 |
```

`tests/`：

```js
it('[FULLNESS-02] 距上次喂食 6h 时饱腹度为 50', () => { ... });
```
...
13. **规格要指明断言哪一层**：写规格时说清断言的是「读取入口的输出」还是「存档里落了什么」。
    有同一性优化的纯函数，这两者会在越界输入上分开 —— 收敛后与当前值相同就不落盘，
    读数对而存档里那个键不存在（`HEALTH-10` / `HEALTH-15` 是活例）。
    写 `expect` 前先回到 `doc.md` 那一行找主语，不要按直觉写期望值。
